...
from backup import generate_image, generate_image_flux, get_data, SQL_RAG
...
if 'human_template' not in st.session_state:
    st.session_state.human_template = ""
...
while st.session_state.human_template.strip().upper() != "STOP":
    iter += 1
    st.write(iter)

    placeholder = st.empty()
    with placeholder.container():
        st.session_state.human_template = st.text_input("Enter some text", key=f'input_{iter}')

    #askAi = st.button("Submit", key=iter * random())
    if st.session_state.human_template:
        messages =[
        {"role": "user", "content": st.session_state.human_template},
        ]
        ...
        if on:
            state_string = get_event(final_state, st.session_state.printed_messages)
            conn = open_connection()
            insert_data(conn, st.session_state.human_template, state_string)
            close_connection(conn)
            st.write("Successfully uploaded response to database!")


        _print_event(final_state, st.session_state.printed_messages)
    else:
        st.stop()
